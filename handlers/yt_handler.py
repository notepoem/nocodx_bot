...
DL_YT1S_API = "https://yt-dlx.vercel.app/api/yt1s?url="
DL_SAVENOW_API = "https://yt-dlx.vercel.app/api/savenow"
DL_FLVTO_API = "https://yt-dlx.vercel.app/api/flvto"
...
def register(bot: TeleBot, custom_command_handler, command_prefixes_list, check_usage_limit=None):
    ...
    async def yt_command(message: Message):
        ...
        if "http" in query and (("youtube.com" in query) or ("youtu.be" in query)):
            ...
            # 1.1 Try YT1S API (Primary)
            try:
                api_url = f"{DL_YT1S_API}{query}"
                resp = await asyncio.to_thread(requests.get, api_url, timeout=15)
                if resp.status_code == 200:
                    data = resp.json()
                    # Check if API returned useful data
                    if data.get("url") or data.get("filename"):
                        video_metadata = {
                            "id": vid_id or "unknown",
                            "title": re.sub(FILENAME_SANITIZE_PATTERN, '', data.get("filename", "YouTube Video")),
                            "duration": str(data.get("duration", "N/A")),
                            "thumbnail": f"https://i.ytimg.com/vi/{vid_id}/hqdefault.jpg" if vid_id else None,
                            "channel": "YouTube",
                            "url": query
                        }
            except Exception as e:
                print(f"[!] YT1S API Failed: {e}")

            # 1.2 Try Savenow API (Fallback 1)
            if not video_metadata:
                try:
                    api_url = f"{DL_SAVENOW_API}?url={query}&format=720p"
                    resp = await asyncio.to_thread(requests.get, api_url, timeout=15)
                    if resp.status_code == 200:
                        data = resp.json()
                        if data.get("success"):
                            video_metadata = {
                                "id": vid_id or "unknown",
                                "title": re.sub(FILENAME_SANITIZE_PATTERN, '', data.get("caption") or data.get("title") or "YouTube Video"),
                                "duration": str(data.get("duration", "N/A")),
                                "thumbnail": data.get("thumbnail"),
                                "channel": data.get("username") or "YouTube",
                                "url": query
                            }
                except Exception as e:
                    print(f"[!] Savenow API Failed: {e}")
            ...
            if video_metadata:
                ...
                markup.add(
                    ...
                    InlineKeyboardButton("🎬 Video (720p/360p)", callback_data=f"download_0_video")
                )
    ...
    async def process_download(bot, chat_id, idx, choice, message, requester=None):
        ...
        try:
            ...
            if choice == "video":
                # 1. Main API: YT1S
                try:
                    api_url = f"{DL_YT1S_API}{video_url}"
                    res = await asyncio.to_thread(requests.get, api_url, timeout=120)
                    data = res.json()
                    
                    if data.get("url"):
                        download_url = data.get("url")
                        if data.get("filename"):
                             title = re.sub(r'[\\/:*?"<>|]', '', data["filename"])
                        success = True
                except Exception as e:
                    print(f"[VIDEO MAIN API ERROR] {e}")

                # 2. Fallback API: Savenow
                if not success:
                    source_api = "savenow"
                    try:
                        api_url = f"{DL_SAVENOW_API}?url={video_url}&format=720"
                        res = await asyncio.to_thread(requests.get, api_url, timeout=120)
                        data = res.json()
                        if data.get("success"):
                            download_url = data.get("download_url") or (data.get("alternative_urls")[0]["url"] if data.get("alternative_urls") else None)
                            success = True
                    except Exception as e:
                        print(f"[VIDEO FALLBACK API ERROR] {e}")

            elif choice == "audio":
                # 1. Primary API: YT1S with format
                try:
                    audio_api_url = f"{DL_YT1S_API}{video_url}&formate=128kbps"
                    res = await asyncio.to_thread(requests.get, audio_api_url, timeout=120)
                    data = res.json()
                    if data.get("url"):
                        download_url = data.get("url")
                        if data.get("filename"):
                    ...
                    print(f"[AUDIO PRIMARY API ERROR] {e}")

                # 2. Existing Fallback 1: Savenow
                if not success:
                    try:
                        savenow_url = f"{DL_SAVENOW_API}?url={video_url}&format=mp3"
                        res = await asyncio.to_thread(requests.get, savenow_url, timeout=120)
                        data = res.json()
                        if data.get("success") and data.get("download_url"):
                             download_url = data.get("download_url")
                             success = True
                        elif data.get("alternative_urls"):
                             download_url = data["alternative_urls"][0].get("url")
                             success = True
                    except: pass

                # 3. Existing Fallback 2: FLVTO
                if not success:
                    try:
                        flvto_url = f"{DL_FLVTO_API}?url={video_url}"
                        res = await asyncio.to_thread(requests.get, flvto_url, timeout=120)
                        data = res.json()
                        if data.get("success") and data.get("download_url"):
                            download_url = data.get("download_url")
                            success = True
            ...
            try:
                ...
                if str(e) == "FileTooLarge":
                    if choice == "video":
                        ...
                        fresh_url = None
                        try:
                            res = await asyncio.to_thread(requests.get, f"{DL_YT1S_API}{video_url}", timeout=60)
                            data = res.json()
                            if data.get("url"):
                                fresh_url = data.get("url")
                        except: pass
                        
                        if not fresh_url:
                             try:
                                 res = await asyncio.to_thread(requests.get, f"{DL_SAVENOW_API}?url={video_url}&format=720", timeout=60)
                                 data = res.json()
                                 if data.get("success"):
                                     fresh_url = data.get("download_url") or (data.get("alternative_urls")[0]["url"] if data.get("alternative_urls") else None)
                             except: pass
                        
                        final_url = fresh_url if fresh_url else download_url
                        await bot.edit_message_text(f"⚠️ File is too large (>50MB).\n\n🔗 <b>Direct Download Link:</b>\n{final_url}", chat_id=chat_id, message_id=wait_msg.message_id, parse_mode="HTML")
                    else:
                        await bot.edit_message_text(f"⚠️ File is too large (>50MB).\n\n🔗 <b>Direct Download Link:</b>\n{download_url}", chat_id=chat_id, message_id=wait_msg.message_id, parse_mode="HTML")
                    return
                else: 
                    raise e
